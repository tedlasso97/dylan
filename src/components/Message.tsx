import { SiOpenai } from "react-icons/si";
import { HiUser } from "react-icons/hi";
import { TbCursorText } from "react-icons/tb";

const Message = (props: any) => {
  const { message } = props;
  const { role, content: text } = message;

  const isUser = role === "user";

  return (
    <div
      className={`group w-full text-white border-b border-gray-600 ${
        isUser ? "bg-gray-900" : "bg-gray-800"
      }`}
    >
      <div className="text-base gap-4 md:gap-6 md:max-w-2xl lg:max-w-xl xl:max-w-3xl flex lg:px-0 m-auto w-full">
        <div className="flex flex-row gap-4 md:gap-6 md:max-w-2xl lg:max-w-xl xl:max-w-3xl p-4 md:py-6 lg:px-0 m-auto w-full">
          <div className="w-8 flex flex-col relative items-end">
            <div className={`relative h-7 w-7 p-1 rounded-sm flex items-center justify-center ${
              isUser ? "bg-cyan-400" : "bg-blue-500"
            }`}>
              {isUser ? (
                <HiUser className="h-4 w-4 text-black" />
              ) : (
                <SiOpenai className="h-4 w-4 text-black" />
              )}
            </div>
            <div className="text-xs flex items-center justify-center gap-1 absolute left-0 top-2 -ml-4 -translate-x-full group-hover:visible !invisible">
              <button
                disabled
                className="text-gray-500"
              ></button>
              <span className="flex-grow flex-shrink-0 text-gray-500">1 / 1</span>
              <button
                disabled
                className="text-gray-500"
              ></button>
            </div>
          </div>
          <div className="relative flex w-[calc(100%-50px)] flex-col gap-1 md:gap-3 lg:w-[calc(100%-115px)]">
            <div className="flex flex-grow flex-col gap-3">
              <div className="min-h-20 flex flex-col items-start gap-4 whitespace-pre-wrap break-words">
                <div className="markdown prose w-full break-words">
                  {!isUser && text === null ? (
                    <TbCursorText className="h-6 w-6 animate-pulse text-cyan-400" />
                  ) : (
                    <p className="text-white">{text}</p>
                  )}
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Message;
